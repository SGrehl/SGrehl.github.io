...
::: callout-tip
VNM-utility values are on a interval scale but not ratio scale, that means they do not have a absolute point 0. We can use this shamelessly to our advantage. For example this means that without loss of generality we can at least replace to parameters in our payoff matrix with any two values we like (as long they respect our ordinal ranking). While it is not always useful to do this with two variables, it is almost always useful to do it at least once with one variable and replace it with 0 (especially, if we are not really interested in this variable).
:::
