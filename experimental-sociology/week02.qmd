...
In other words, whenever $x$ is at least as preferred as $y$, $u$ must assign to $x$ a value at least as large as the one it assigns to $y$. In Table @tbl-utility-ordinal we see that the ordinal preferences $x\succ y \succ z$ are represented by the utility function $u$ and $v$, but not by $h$ as $h(y)>h(x)$ contradicts with $x\succ y$.
...
This is often the central question we want to answer as researchers: under which conditions does an actor choose one action over another? So, now we can use the information from Table @tbl-decision-ticket, to replace these terms with:
...
In game theory, we refer to the actors as **players**. Let $N = \{1, 2, 3, \dots, n\}$ be the set of players. Each player $i \in N$ has a set of available **actions** $A_i$, and a **Von Neumann-Morgenstern utility function** $u_i$ that assigns a utility value to every possible outcome. We denote the set of all players except player $i$ as $-i$, that is, $-i=N\{i}$.
...
To illustrate the basic structure of a game, let’s consider a classic **2-player game** where each player has two strategies. We represent this situation in a **payoff matrix**. Each cell of the table shows the utility (or payoff) for both players, given their chosen actions.
...
A **Nash equilibrium** is a set of strategies—one for each player—such that **no player has an incentive to unilaterally deviate**. In other words, given the strategies chosen by the others, each player is doing the best they can. Formally, a **action profiles** $a^*=(a_1^*, a_2^*, \dots, a_n^*)$ is a Nash equilibrium if for every player $i$:
...
Let’s go back to our example from Table @tbl-game-pd and analyze it.
...
Even though both players would be better off if they both cooperated (3, 3), cooperation is **not** stable: each has an incentive to unilaterally deviate if the other is cooperating. This tension between individual rationality and collective outcome is what makes game theory so powerful—and often so frustrating.

### Mixed Nash Equilibrium

So far, we have focused on **pure strategies** (specific actions), where each player selects a single action with certainty. However, there are many games—especially competitive ones—where no pure strategy Nash equilibrium exists. In such cases, we extend our analysis to include **mixed strategies**.
...
$$
\mathrm{EU}(\sigma^*, \sigma_{-i}^*) \geq \mathrm{EU}(\sigma, \sigma_{-i}^*) \quad \text{for all } \sigma_i \in \Delta(A_i).
$$

This means that given the other players' mixed strategies, player $i$ has no incentive to change their own.
...
Let each player choose H with probability 0.5 and T with probability 0.5. Then:

-   Player 1 cannot improve their expected payoff by deviating, since they always face a 50/50 mix from Player 2.
-   The same goes for Player 2.

> Mixed strategy equilibrium: Both players randomize uniformly (50/50) over Heads and Tails.

This equilibrium reflects a fundamental idea in game theory: sometimes, **deliberate unpredictability** is the optimal strategy.

### Why Mixed Strategies Matter

Mixed strategy equilibria are not just mathematical curiosities—they're crucial for analyzing a wide range of real-world scenarios, including auctions, sports, warfare, and competitive markets. Moreover, Nash’s famous existence theorem guarantees that **every finite game has at least one Nash equilibrium**, possibly in mixed strategies.

In the next sections, we’ll explore further refinements and solution concepts, but for now, it’s important to remember: when pure strategies fail to stabilize, **randomization can restore equilibrium**.
